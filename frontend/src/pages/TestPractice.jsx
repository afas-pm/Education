...
const TestPractice = () => {
    ...
    const getIcon = (iconName) => {
        const cls = "w-7 h-7";
        switch (iconName) {
            case "book-open":  return <BookOpen    className={`${cls} text-[#017CBA]`} strokeWidth={2} />;
            case "check":      return <CheckCircle className={`${cls} text-[#10B981]`} strokeWidth={2} />;
            case "bar-chart":  return <BarChart3   className={`${cls} text-[#3B82F6]`} strokeWidth={2} />;
            case "clock":      return <Clock       className={`${cls} text-[#F97316]`} strokeWidth={2} />;
            default:           return <BookOpen    className={`${cls} text-[#017CBA]`} strokeWidth={2} />;
        }
    };
                ...
                <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-14">
                        {testStats.map((stat) => (
                            <div
                                key={stat.label}
                                className="bg-white px-5 py-5 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.06)] flex items-center gap-5 border border-gray-100"
                            >
                                <div className="shrink-0">
                                    {getIcon(stat.icon)}
                                </div>
                                <div className="flex flex-col">
                                    <p className="text-[28px] font-bold text-[#1E293B] leading-none">
                                        {stat.value}
                                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {availableTests.map((test) => (
                            <div
                                key={test.id}
                                className="bg-white rounded-xl border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] transition-all duration-200 flex flex-col justify-between"
                            >
                                <div className="p-6 flex flex-col gap-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="px-3 py-0.5 bg-white text-[#1E293B] border border-gray-300 rounded-full text-[12px] font-semibold">
                                            {test.difficulty}
                                        </span>
                                        <span className="px-3 py-0.5 bg-[#F97316] text-white rounded-full text-[12px] font-semibold">
